...
此專案為基於 ASP.NET Core 的後端 API，採用 .NET 10.0 框架，專注於高效能與可擴展性。技術重點包括：
...
5. **API 文件生成**：集成 Swagger 提供文件與測試介面。
6. **模組化設計**：分層架構提升代碼可維護性。
7. **環境配置**：使用 `appsettings*.json` 管理設定，JSON 序列化採用 `System.Text.Json`。
